...
`spex-3.07.02-Source.tar.bz2 <https://doi.org/10.5281/zenodo.1924563>`_.
Unpack it in a convenient directory::

    unix:~/Software> tar xvfj spex-3.07.02-Source.tar.bz2
...
where CMakeLists.txt is located (mind the dot)::
    ...
    unix:~/Software/SPEX-3.07.02-Source> cmake .

If no errors occured and all libraries were found, then type 'make'::

    unix:~/Software/SPEX-3.07.02-Source> make

When the program needs to be installed system wide, then execute::

    unix:~/Software/SPEX-3.07.02-Source> sudo make install
...
Select a different fortran compiler::
    ...
    unix:~/Software/SPEX-3.07.02-Source> cmake . -DCMAKE_Fortran_COMPILER=ifort
...
Install SPEX at a different location in the 'make install' step::
    ...
    unix:~/Software/SPEX-3.07.02-Source> cmake . -DCMAKE_INSTALL_PREFIX=/home/user/software
...
The use of the SPEX libraries in the source tree can be forced::
    ...
    unix:~/Software/SPEX-3.07.02-Source> cmake . -DPGPLOT=YES
